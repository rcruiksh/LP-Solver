...
class LP:
        ...
        for row in range(len(self.matrix)):
            for col in range(len(self.matrix[0])):
                if row == 0 and col > 0:
                    ...
                    self.dictionary[row][col] = Node(row, col, self.matrix[row][col], float('inf'))
    ...
    def getOptVarVals(self):
        ...
        return [var.mag for var in opt_vars]
    ...
    def primalPivot(self):
        ...
        leaving_row[entering.col] = Node(leaving.row, entering.col, -1, float('inf'))
        # self.printDictionary()
        ...
        for row in range(len(self.dictionary)):
            if row != leaving.row:
                ...
                for col in range(len(self.dictionary[row])):
                    ...
                    # elif col == entering.col:
                    #     new_row.append(Node(row, col, expansion[col], self.dictionary[row][col].number))
                    # else:
                    #     new_row.append(self.dictionary[row][col] + (expansion[col] * self.dictionary[row][col].mag))
                    if col == entering.col:
                        new_row.append(Node(row, col, expansion[col], self.dictionary[row][col].number))
                    else:
                        new_row.append(self.dictionary[row][col] + expansion[col])
